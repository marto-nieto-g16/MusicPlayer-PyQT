...
class MusicTableView(QTableView):
    def __init__(self, type, parent=None):
        ...
        self.playAction = QAction(u'Play', self)
        self.deleteAction = QAction(u'Borrar', self)
        self.favAction = QAction(u'Coleccion', self)
        self.openFolderAction = QAction(u'Abrir', self)
    ...
    def contextMenuEvent(self, event):
        ...
        if self.type == 'all' and self.model.itemFromIndex(index):
            if self._parent.musicList[self.model.itemFromIndex(index).row()] in self._parent.favList:
                self.favAction.setText(u"Cancelar colección")
            else:
                self.favAction.setText(u"Coleccion")
