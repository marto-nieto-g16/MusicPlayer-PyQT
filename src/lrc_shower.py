...
class LRCShower(QListView):
    ...
    def __init__(self, parent=None):
        ...
        self.openAction = QAction(u'Abrir', self)
        self.downloadAction = QAction(u'Descargarr', self)
        self.deleteAction = QAction(u'Borrar', self)
        ...
        self.waitForLrcLabel.setText(u"Arrastra el archivo de la letra hacia la apertura\n   O haga clic derecho para descargar")
        ...
        self.downloadFailedLabel.setText(u"   La descarga de la letra falló.，\nPor favor, abre el archivo de letras local")
        self.downloadFailedLabel.setGeometry(180, 110, 200, 200)
        self.downloadFailedLabel.setHidden(True)

    def openLRC(self, path=None):
        if not path:
            path = QFileDialog.getOpenFileName(self, u'Abrir', self.lastOpenedPath, u'Archivo de letras (*.lrc)')
            if path:
